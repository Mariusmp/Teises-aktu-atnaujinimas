...
class TestApp(unittest.TestCase):
    ...
    def test_root_endpoint(self, mock_template_response):
        ...
        # Provide correct basic auth credentials
        response = self.client.get("/", auth=("admin", os.getenv("APP_PASSWORD")))
    ...
    def test_root_endpoint_unauthorized(self):
        """Test the root endpoint returns 401 without correct credentials"""
        response = self.client.get("/")
        self.assertEqual(response.status_code, 401)

        response_wrong_user = self.client.get("/", auth=("wrong", os.getenv("APP_PASSWORD")))
        self.assertEqual(response_wrong_user.status_code, 401)
    ...
    def test_websocket_unauthorized(self):
        """Test the websocket endpoint closes on missing or wrong token"""
        ...
        with self.assertRaises(WebSocketDisconnect) as e:
            with self.client.websocket_connect("/ws?token=wrong_token") as websocket:
                pass
        self.assertEqual(e.exception.code, 1008)
